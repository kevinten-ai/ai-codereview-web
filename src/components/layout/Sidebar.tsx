...
const navSections = [
  {
    items: [
      ...
      },
      {
        href: '/reviews',
        label: 'Reviews',
        icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5" /></svg>,
        badge: '23',
      },
    ],
  },
  ...
  return (
    ...
    <aside className="sidebar fixed left-0 top-0 z-30 hidden h-screen w-[220px] flex-col md:flex">
      ...
      <div className="h-14 flex items-center gap-2.5 px-5">
        ...
        <span className="text-[13px] font-semibold text-white tracking-tight">CodeReview</span>
      </div>
      ...
      <nav className="flex-1 py-2 px-3 space-y-4 overflow-y-auto">
        {navSections.map((section, si) => (
          <div key={si}>
            {section.label && (
              <div className="px-3 mb-1 text-[11px] font-medium text-zinc-600 uppercase tracking-wider">
                {section.label}
              </div>
            )}
            <div className="space-y-0.5">
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={cn('nav-icon transition-colors', isActive ? 'text-violet-400' : 'text-zinc-600 group-hover:text-zinc-400')}>
                      {item.icon}
                    </span>
      ...
      <div className="p-3 border-t border-white/5">
        <div className="flex items-center gap-2.5 px-3 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-medium text-zinc-200 truncate">Kevin</p>
